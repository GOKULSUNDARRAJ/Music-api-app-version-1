...
class HomeDevotionalTab extends StatefulWidget {
  const HomeDevotionalTab({super.key});

  @override
  State<HomeDevotionalTab> createState() => _HomeDevotionalTabState();
}

class _HomeDevotionalTabState extends State<HomeDevotionalTab> {
  ...
  Future<void> _fetchDevotionalData() async {
    try {
      ...
      final response = await http.get(
        Uri.parse('https://music-app-api-1.onrender.com/api/devotional'),
      ...
      decoration: BoxDecoration(
        ...
        image: DecorationImage(
          ...
          onError: (exception, stackTrace) {
            debugPrint('home_devotional_tab: Failed to load image $url — $exception');
          },
        ),
      ),
    );
                  ...
                  GestureDetector(
                    onTap: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => SectionSeeAllScreen(section: section),
                        ),
                      );
                    },
                    child: Row(
  ...
  Widget build(BuildContext context) {
    ...
    return RefreshIndicator(
      onRefresh: _fetchDevotionalData,
